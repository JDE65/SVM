...
train_round = 1;
train_effic = zeros(10, 3);
for trs = [250, 500, 750, 1000, 1250, 1500, 1750, 2000, 2250, 2500]
  train_size = trs;
  cv_size_init = trs + 1;      
...
train_effic(train_round, 1) = m_train;
train_effic(train_round, 2) = F1;
train_effic(train_round, 3) = F1t;
train_round =+ 1;

plot(m_train, F1, "@31",'LineWidth', 1, m_train, F1t, "@13")
% plot(train_effic(:, 1),train_effic(:, 2), 'k+','LineWidth', 1, 'F1 trained', 7)
% hold on;
% plot(train_effic(:, 1),train_effic(:, 3), 'ko','LineWidth', 1, 'F1 trained', 7)
hold on;
%% Printing results
